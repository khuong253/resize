...
from helpers.util import convert_xywh_to_ltrb
...
def sparse_to_dense(
    ...
    input_bbox, output_bbox = bbox[..., :4], bbox[..., 4:]
    label, mask = to_dense_batch(batch.y, batch.batch)

    if remove_canvas:
        input_bbox = input_bbox[:, 1:].contiguous()
        output_bbox = output_bbox[:, 1:].contiguous()
        label = label[:, 1:].contiguous() - 1  # cancel +1 effect in transform
        label = label.clamp(min=0)
        mask = mask[:, 1:].contiguous()

    padding_mask = ~mask
    bbox = {"input_bbox": input_bbox, "output_bbox": output_bbox}
