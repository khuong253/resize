...
from helpers.mask import sample_mask
from helpers.util import batch_topk_mask

MAX_PARTIAL_RATIO = 0.3
COND_TYPES = [
    "c",  # given category, predict position/size (C->P+S)
    "cwh",  # given category/size, predict position (C+S->P)
    "cwhfirst",  # given category and first element size, predict position and other sizes (C+S(first)->P+S(other))
...
def get_cond(
    ...
    elif cond_type in ["c", "cwh", "cwhfirst", "relation"]:
        vars = {"c": "c", "cwh": "cwh", "cwhfirst": "cwh", "relation": "c"}
        keep = torch.full((B, S), False)
        if "bos" in special_keys:
            seq_ind =  (torch.arange(S).view(1, S) - 1)
            attr_ind = seq_ind % C
            attr_ind[:, 0] = -1  # dummy id for BOS
            keep[:, 0] = True
        else:
            seq_ind = torch.arange(S).view(1, S)
            attr_ind = seq_ind % C
...
def duplicate(seq: Tensor, batch_size: int) -> Dict:
    flag = seq.size(0) == 1
    flag &= batch_size > 1
    if flag:
        sizes = [batch_size,]
        sizes += [1 for _ in range(seq.dim() - 1)]
        seq = seq.repeat(sizes)
    return seq